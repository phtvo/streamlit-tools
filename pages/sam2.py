...
if uploaded_file and uploaded_file.name != st.session_state.current_video:
    ...
    tfile = tempfile.NamedTemporaryFile(delete=True, suffix=".mp4", prefix="input")
...
def get_annotated_image(base_frame, objs: list):
    ...
    for obj in objs:
        ...
        # for (x, y), label in zip(obj["points"], obj["labels"]):
        #     point_color = "green" if label == 1 else "red"
        #     draw.ellipse([(x - 4, y - 4), (x + 4, y + 4)], fill=point_color)
...
if st.session_state.video_frames:
    ...
    with mid_col2:
        ...
        st.text_area("Objects", list_input_dict)
        #for obj in st.session_state.objects:
    ...
    if st.button("Submit to Track") and uploaded_file:
        ...
        for each in st.session_state.objects:
            list_input_dict.append({k: v for k, v in each.items() if k != "mask"})
        tracked_frames: Iterator[dt.Frame] = model.track(video=dt.Video(bytes=uploaded_file.read()), list_dict_inputs=list_input_dict)
        ...
        with st.spinner("Tracking.."):
            for trk_frame in tracked_frames:
                ...
                st.session_state.fps = round(sum(total_track_time)/len(total_track_time), 3)
